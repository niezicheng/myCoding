...
function saveOrUpdateArticle(obj,cid,page,option){
	myAjax('/manager/article/saveOrUpdateArticle','POST',function(res){
		...
		if(option === '新增'){
			console.log(page,'****');
			findArticle(getPage,page,pageSize,cid);
...
function batchDeleteArticle(arr,cid){
	...
	myAjax('/manager/article/batchDeleteArticle','POST',function(res){
		//查询相应所属栏目cid的文章信息
		// findArticle(findArtFun,page,pageSize,cid,1);
		// console.log(getPage,page,pageSize,cid,'&&&&&&&&');

		findArticle(getPage,page,pageSize,cid,cid,1);
	},function(error){
		console.log(error);
	},{ids:arr.toString()});
}
